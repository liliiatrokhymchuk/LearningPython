...
phoneNumRegex = re.compile(r'(\d\d\d)-(\d\d\d-\d\d\d\d)')
matchobject = phoneNumRegex.search('My number is 415-555-4242.')
print(matchobject.group(1))  #(\d\d\d) is group 1. .group(0) or .group() return entire matching set
...
print (matchobject1.group())  #if multiple values matches, the first one only will be return this way
